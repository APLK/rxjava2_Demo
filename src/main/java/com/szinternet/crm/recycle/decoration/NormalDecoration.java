package com.szinternet.crm.recycle.decoration;
    ...
    public void getItemOffsets(Rect outRect, View itemView, RecyclerView parent, RecyclerView.State state) {
        ...
        if (!mIncludeHeader) {
            if (parent.getAdapter() instanceof RecyclerArrayAdapter) {
                ...
                if (pos >= headerCount + dataCount) {
                    return;
                }
                if (pos >= headerCount) {
                    if (pos == headerCount + dataCount) {
                        pos = pos - headerCount - footerCount;
                    } else {
                        pos -= headerCount;
                    }
                }
                LogUtils.i("1", "pos=" + pos + ",headerCount=" + headerCount + ",footerCount=" + footerCount + ",dataCount=" + dataCount);
            }
        }
        ...
        if (pos != RecyclerView.NO_POSITION && (pos == 0 || !curHeaderName.equals(getHeaderName(pos - 1)))) {//如果当前位置为0，或者与上一个item头部名不同的，都腾出头部空间
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (!mIncludeHeader) {
                if (recyclerView.getAdapter() instanceof RecyclerArrayAdapter) {
                    ...
                    int footerCount = ((RecyclerArrayAdapter) recyclerView.getAdapter()).getFooterCount();
                    int dataCount = ((RecyclerArrayAdapter) recyclerView.getAdapter()).getCount();
                    if (pos < headerCount) {
                        continue;
                    }
                    if (pos > headerCount + dataCount) {
