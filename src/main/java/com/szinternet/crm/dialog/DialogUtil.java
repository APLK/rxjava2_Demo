...
import com.szinternet.crm.adapter.UpdateTextAdapter;
...
public class DialogUtil {
    ...
    public void updataDlg(String surebtstr, final DialogInterface inter, String version, String clientText) {
        ...
        if (clientText != null) {
            String[] updateInfos = clientText.split("<b>");
            List<String> updateInfosList = new ArrayList<>();
            for (int i = 0; i < updateInfos.length; i++) {
                updateInfosList.add(updateInfos[i]);
            }
            //更新信息
            ListView update_infos = (ListView) view.findViewById(R.id.update_infos);
            update_infos.setAdapter(new UpdateTextAdapter(mContext, updateInfosList));
        }
        ...
        upApkDlg.setCanceledOnTouchOutside(true);
    ...
    public void billItemDlg(String repayStatus, final DialogInterface inter, String date, String tbody) {
        ...
        LogUtils.e("tbdoy:", tbody + "===========tbody");

        mTvDlgBillTbody.setText(tbody);

        // 判断是否逾期
        if (TextUtils.equals(repayStatus, "502103")) {
            mLlDlgBill.setBackgroundDrawable(mContext.getResources().getDrawable(R.mipmap.img_bill_item_balck));
            ...
            mLlDlgBill.setBackgroundDrawable(mContext.getResources().getDrawable(R.mipmap.img_bill_item_red));
            //判断是否还款
            if (isHuanKuan) {
